...
| `bodyweight_entries` | журнал веса тела: одно измерение на дату, под график | 1 |
...
### 2.7 Вес тела: журнал — источник, профиль — производная

```sql
constraint bodyweight_entries_day_uniq unique (profile_id, measured_on)
```

Одно измерение на дату: повторная запись за тот же день исправляет прежнюю, а не
плодит вторую точку на графике. Индекс этого констрейнта закрывает и запрос
«последнее измерение профиля», отдельного не нужно.

`profiles.bodyweight_kg` остаётся текущим весом (его читают упражнения с собственным
весом и фаза 2), но выводится из журнала: после каждой вставки, правки и удаления
триггер `sync_profile_bodyweight()` кладёт в профиль вес самой свежей записи, а пустой
журнал обнуляет колонку.

**Направление синхронизации ровно одно, журнал → профиль.** Обратное — триггер на
`profiles`, дописывающий измерение, — выглядит симметрично, но ломается на двух
сценариях: два триггера пишут друг в друга по кругу, а исправленное задним числом
измерение порождает фантомную запись за сегодня. Поэтому вес правится только через
журнал; клиент, сохраняя профиль, сам дописывает измерение сегодняшним днём.

`measured_on` — `date`, и приходит она с клиента, а не из `current_date`: сервер живёт
в UTC, и вес, записанный в Москве до трёх ночи, лёг бы во вчерашний день.

### 2.8 Очки не считаются в фазе 1
...
одним файлом `..._rls_policies.sql` последним по порядку. Исключение — таблицы,
добавленные после того, как этот файл уже применён на проекте: их политика лежит
в собственной миграции (так живёт `bodyweight_entries`), потому что править
применённую миграцию задним числом нельзя.
...
| `bodyweight_entries` | владелец | владелец | владелец | владелец |
